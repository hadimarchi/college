...
ABCD recurseCallContigSum(RAIter first, RAIter last){
  ...
  if (iter_range == 1){return ABCD(max(*first, max(*first+*last, *last)),
...
int maxWithLast(const ABCD & first_half, const ABCD & second_half){
  return max(0, max(second_half.max_with_last, first_half.max_with_last + second_half.full_sum));
}

int maxWithFirst(const ABCD & first_half, const ABCD & second_half){
  return max(0, max(first_half.max_with_first, first_half.full_sum + second_half.max_with_first));
}
